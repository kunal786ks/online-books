import { COMMENT_ICON, LIKE_ICON } from "@/assests/icons";
import ImageComponent from "@/component/atom/imageComponent";
import { Box, Container, Text } from "@chakra-ui/react";

const Blogs = () => {
  const blogs: any = [
    {
      name: "This is blog 1 and the data for the blogs frot h dbdfgf",
      like: 12,
      author: "abcd",
      comment: 4,
    },
    {
      name: "this is blog 1 ",
      like: 12,
      author: "abcd",
      comment: 4,
    },
    {
      name: "this is blog 1",
      like: 12,
      author: "abcd",
      comment: 4,
    },
  ];
  return (
    <Container m="20px">
      <Text fontSize="20px" color="rgb(85, 96, 129)">
        Popular Blogs
      </Text>
      <Box mt="20px">
        {blogs?.map((blog: any, index: number) => (
          <Box
            key={index}
            display="flex"
            justifyContent="space-between"
            alignItems="center"
            mt="10px"
            gap="20px"
          >
            <ImageComponent
              height="135px"
              width="160px"
              alt={blog.name}
              src="data:image/png;base64,"
            />
            <Box
              display="flex"
              justifyContent="space-between"
              alignItems="center"
              flexDir="column"
              h="135px"
              w="100%"
            >
              <Text style={{
                display:""
              }} fontSize="20px" color="gray" fontWeight="600px" mr="auto">
                {blog.name}
              </Text>
              <Box mr="auto">
                <Text color="gray">Published By {blog.author}</Text>
                <Text color="lightgray" display="flex" alignItems="center" gap="10px">
                  <LIKE_ICON /> {blog.like}
                  <COMMENT_ICON /> {blog.comment}
                </Text>
              </Box>
            </Box>
          </Box>
        ))}
      </Box>
    </Container>
  );
};

export default Blogs;
